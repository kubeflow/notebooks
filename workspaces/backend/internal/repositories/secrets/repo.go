...
func (r *SecretRepository) CreateSecret(ctx context.Context, namespace string, secretCreate *models.SecretCreate) (*models.SecretCreate, error) {
	...
	k8sSecret := secretCreate.ToKubernetesSecret(namespace, userEmail)
...
func (r *SecretRepository) UpdateSecret(ctx context.Context, namespace string, secretName string, secretUpdate *models.SecretUpdate) (*models.SecretUpdate, error) {
	...
	// TODO: Get actual user email from context when implementing real K8s interaction
	userEmail := "mock@example.com"
	k8sSecret := secretUpdate.ToKubernetesSecret(currentSecret, userEmail)
	...
	return []*corev1.Secret{
		{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					"notebooks.kubeflow.org/created-by": "admin@example.com",
					"notebooks.kubeflow.org/created-at": time.Date(2024, 1, 15, 10, 30, 0, 0, time.UTC).Format(time.RFC3339),
					"notebooks.kubeflow.org/updated-by": "admin@example.com",
					"notebooks.kubeflow.org/updated-at": time.Date(2024, 2, 20, 14, 45, 0, 0, time.UTC).Format(time.RFC3339),
				},
			},
		...
		},
		{
			ObjectMeta: metav1.ObjectMeta{
				...
				Namespace: namespace,
				Labels: map[string]string{
					"notebooks.kubeflow.org/can-update": "false",
					"notebooks.kubeflow.org/can-mount":  "true",
				},
				Annotations: map[string]string{
					"notebooks.kubeflow.org/created-by": "devops@example.com",
					"notebooks.kubeflow.org/created-at": time.Date(2024, 1, 10, 9, 15, 0, 0, time.UTC).Format(time.RFC3339),
					"notebooks.kubeflow.org/updated-by": "devops@example.com",
					"notebooks.kubeflow.org/updated-at": time.Date(2024, 1, 10, 9, 15, 0, 0, time.UTC).Format(time.RFC3339),
				},
			},
			Type: corev1.SecretTypeOpaque,
			Data: map[string][]byte{
		...
		},
		{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					"notebooks.kubeflow.org/created-by": "security@example.com",
					"notebooks.kubeflow.org/created-at": time.Date(2024, 3, 5, 16, 20, 0, 0, time.UTC).Format(time.RFC3339),
					"notebooks.kubeflow.org/updated-by": "security@example.com",
					"notebooks.kubeflow.org/updated-at": time.Date(2024, 3, 12, 11, 30, 0, 0, time.UTC).Format(time.RFC3339),
				},
			},

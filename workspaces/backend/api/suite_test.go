...
var _ = BeforeSuite(func() {
	...
	// NOTE: we use the `k8sClient` rather than `k8sManager.GetClient()` to avoid race conditions with
	//       the cached client. it is uncached, so it also serves as the apiReader.
	a, err = NewApp(&config.EnvConfig{}, appLogger, k8sClient, k8sClient, imageSourceConfigMapClient, k8sManager.GetScheme(), reqAuthN, reqAuthZ, clientset)
	Expect(err).NotTo(HaveOccurred())

...
// This drives the filter UI and logic, making it easy to add/remove filters.
const filterConfigs = [
  { key: 'name', label: 'Name', placeholder: 'Filter by name' },
  { key: 'kind', label: 'Kind', placeholder: 'Filter by kind' },
  { key: 'image', label: 'Image', placeholder: 'Filter by image' },
...
type FilterLabel = (typeof filterConfigs)[number]['label'];
        ...
        >
          <SelectList>
            {filterConfigs.map(({ key, label }) => (
              <SelectOption key={key} value={label}>
                {label}
              </SelectOption>
            ))}
          </SelectList>

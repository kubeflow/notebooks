...
import { MAX_DISPLAY_NAME_LENGTH, validateDisplayName } from '~/app/pages/Workspaces/Form/helpers';
...
> = ({
  ...
  const [isSlugManuallyEdited, setIsSlugManuallyEdited] = useState(
    Boolean(
      mode === 'update' || (selectedProperties.workspaceName && !selectedProperties.displayName),
    ),
  );
    ...
    (value: string) => {
      if (!isSlugManuallyEdited && mode !== 'update') {
        ...
        if (onDisplayNameChange) {
          ...
          onSelect({ ...selectedProperties, displayName: value, workspaceName: nextWorkspaceName });
          onWorkspaceNameChange(nextWorkspaceName);
        }
      } else if (onDisplayNameChange) {
        onDisplayNameChange(value);
      } else {
        onSelect({ ...selectedProperties, displayName: value });
      }
    },
    [

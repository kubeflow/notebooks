...
var _ = Describe("Workspace NetworkPolicy Generation", func() {
	...
	// The activity probe connects to the workspace pod from the controller, so
	// the controller must be admitted alongside the routing layer.
	It("should also allow the controller when configured", func() {
		reconciler.Config.WorkspaceNetworkPolicy.ControllerNamespace = "kubeflow-workspaces"
		reconciler.Config.WorkspaceNetworkPolicy.ControllerPodSelector = map[string]string{"app": "workspaces-controller"}
		networkPolicy := reconciler.generateNetworkPolicy(workspace)

		Expect(networkPolicy.Spec.Ingress).To(HaveLen(1))
		Expect(networkPolicy.Spec.Ingress[0].From).To(HaveLen(2))
		peer := networkPolicy.Spec.Ingress[0].From[1]
		Expect(peer.NamespaceSelector.MatchLabels).To(HaveKeyWithValue(namespaceNameLabel, "kubeflow-workspaces"))
		Expect(peer.PodSelector.MatchLabels).To(HaveKeyWithValue("app", "workspaces-controller"))
	})

...
// Workspace's pods to the configured routing layer and to the controller
// itself, whose activity probe connects to the pod directly.
//
...
func (r *WorkspaceReconciler) generateNetworkPolicy(workspace *kubefloworgv1beta1.Workspace) *networkingv1.NetworkPolicy {
	...
	peers := []networkingv1.NetworkPolicyPeer{
		networkPolicyPeer(cfg.IngressNamespace, cfg.IngressPodSelector),
	}
	if cfg.ControllerNamespace != "" {
		peers = append(peers, networkPolicyPeer(cfg.ControllerNamespace, cfg.ControllerPodSelector))
	}
...
// networkPolicyPeer selects pods in a namespace, or the whole namespace when
// podSelector is empty.
func networkPolicyPeer(namespace string, podSelector map[string]string) networkingv1.NetworkPolicyPeer {

			...
			if enableExtAuthz {
				...
				verifyExternalAuthFilter := func(g Gomega) {
					...
					for _, filter := range filters {
						g.Expect(filter).To(Equal("ExternalAuth/workspaces-backend"),
							"every rule must start with an ExternalAuth filter pointing at the backend's authorization endpoint")
					}
				}
